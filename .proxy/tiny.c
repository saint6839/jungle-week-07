...
void doit(int connfd)
{
  ...
  is_static = parse_uri(uri, filename, cgiargs); // uri, filename, cgiargs 할당
  ...
  if (is_static)
  {
    // 정적 컨텐츠일때 파일 형식이 일반 파일이 아니거나, 사용자에게 읽기 권한이 없다면
  ...
  {
    // 동적 컨텐츠일때 파일 형식이 일반 파일이 아니거나, 사용자에게 실행 권한이 없다면
