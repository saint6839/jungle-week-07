...
void clienterror(int fd, char *cause, char *errnum, char *shortmsg,
  ...
  Rio_writen(fd, buf, strlen(buf)); // Rio_written() 한번 호출시마다 버퍼 비워짐
...
int parse_uri(char *uri, char *filename, char *cgiargs) {
  ...
  if (!strstr(uri, "cgi-bin")) { // 만약 동적이 아닌 정적 컨텐츠에 대한 요청이라면
    strcpy(cgiargs, "");
    strcpy(filename, "."); // .
    strcat(filename, uri); // .{uri}

    if (uri[strlen(uri)-1] == '/') // uri의 끝이 / 라면, filename에 home.html을 이어 붙임
      strcat(filename, "home.html"); // .{uri}/home.html
    return 1;
  } else { // 동적 컨텐츠에 대한 요청이라면
    ptr = index(uri, '?');
    if (ptr) {
...
void serve_dynamic(int fd, char *filename, char *cgiargs) {
  ...
  Rio_writen(fd, buf, strlen(buf)); // 클라이언트에 성공을 먼저 알린다

  /* 자식 프로세스 생성 성공 시*/
  if (Fork() == 0) {
    /* 부모 서버는 모든 CGI 변수들을 이 프로세스에 세팅한다.*/
    setenv("QUERY_STRING", cgiargs, 1);
    Dup2(fd, STDOUT_FILENO); // stdout을 클라이언트에게 redirect 시킨다.
    Execve(filename, emptylist, environ); // CGI 프로그램 실행, 자식 프로세스이기 때문에 부모에서 열렸던 파일이나 환경변수에도 동일하게 접근 가능
  }
...
void serve_static(int fd, char *filename, int filesize, char *method) {
  ...
  if (!strstr(method, "HEAD")) {
    ...
    Close(srcfd);
    Rio_writen(fd, srcp, filesize); // 클라이언트와 연결된 연결 식별자(fd)에 주소 srcp부터 filesize byte 만큼의 내용을 복사 -> 클라이언트에게 응답 전달
    free(srcp);
  }
}
