 ...
 */
#include "csapp.h"
#define KEEP_ALIVE_TIMEOUT 5  // 5초로 설정
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    *connfd_ptr = Accept(listenfd, (SA *)&clientaddr,
                    &clientlen);  
...
int parse_uri(char *uri, char *filename, char *cgiargs) {
  ...
  if (!strstr(uri,
              "cgi-bin")) {  // 만약 동적이 아닌 정적 컨텐츠에 대한 요청이라면
    strcpy(cgiargs, "");
    strcpy(filename, ".");  // .
...
void serve_dynamic(int connfd, char *filename, char *cgiargs) {
  ...
  if (Fork() == 0) {
    ...
    Execve(filename, emptylist,
           environ);  // CGI 프로그램 실행, 자식 프로세스이기 때문에 부모에서
                      // 열렸던 파일이나 환경변수에도 동일하게 접근 가능
  }
...
void serve_static(int connfd, char *filename, int filesize, char *method) {
  ...
  if (!strstr(method, "HEAD")) {
    ...
    Rio_readn(srcfd, srcp, filesize);  // 확보한 heap 공간에 srcfd에서 filesize
                                       // 만큼을 srcp로 읽어온다.
    Close(srcfd);
    Rio_writen(connfd, srcp,
               filesize);  // 클라이언트와 연결된 연결 식별자(fd)에 주소
                           // srcp부터 filesize byte 만큼의 내용을 복사 ->
                           // 클라이언트에게 응답 전달
    free(srcp);
  }
}
